...
import {AiFillEnvironment, AiOutlineMail, AiOutlineBarChart, AiOutlineFileText} from "react-icons/ai"
...
const App =() =>{
  ...
  const Menus = [
    ...
    {title: "profit", spacing:true ,icon: <BsPerson/>}
  ];
  return (
    <div className="flex">
        <div className={`bg-dark-purple h-screen p-5 pt-8 ${open ? "w-72":"w-20"} duration-300 relative`}>
          ...
          <div className="inline-flex">
            <AiFillEnvironment className={`bg-amber-300 text-4xl rounded cursor-pointer block float-left mr-2 duration-500 ${open && "rorate-[360deg]"}`}/>
            <h1 className={`text-white origin-left font-medium text-2xl duration-300 ${!open && "scale-0"}`}>NLP LAB</h1>
          </div>
